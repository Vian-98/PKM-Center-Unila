...
function App() {
  ...
  return <>
    <header><a className="brand" href="#beranda"><span className="brand-mark">P</span><span>PKM <i>Center</i><small>UNIVERSITAS LAMPUNG</small></span></a><nav><a href="#tentang">Tentang</a><a href="#lintasan">Lintasan PKM</a><a href="#karya">Karya</a></nav>{state.user ? <button className="profile" onClick={logout}>{state.user.name.split(' ')[0]} <span>{state.user.role}</span></button> : <button className="login-trigger" onClick={() => setOpen(true)}>Masuk <b>→</b></button>}</header>
    <main id="beranda">
